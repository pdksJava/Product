...
public class FazlaCalismaRaporHome extends EntityHome<DepartmanDenklestirmeDonemi> implements Serializable {
	...
	private boolean ikRole;
	private boolean talepGoster;
	private Boolean departmanBolumAyni = Boolean.FALSE;
	private Boolean tekSirket;
	private Boolean modelGoster = Boolean.FALSE;
	private Boolean kullaniciPersonel = Boolean.FALSE;
	private Boolean fazlaMesaiSayfa = Boolean.TRUE;
	private int ay;
	private int yil;
	private int maxYil;
	private int maxFazlaCalismaGun;
	...
	private String raporSecim;
	private String raporAdi;
	private String sicilNo = "";
	private String excelDosyaAdi;
	private Long seciliEkSaha3Id;
	private Long sirketId = null;
	private Long departmanId;
	private Long gorevTipiId;
	private Long tesisId;
	private Tanim gorevYeri;
	private Tanim seciliBolum;
	private Tanim ekSaha4Tanim;
	private byte[] excelData;
	private List<SelectItem> pdksSirketList;
	...
	private List<VardiyaGun> vardiyaGunList;
	private List<VardiyaGun> vardiyaGunPerList;
	private VardiyaGun vardiyaGun;
	private Font fontBold;
	private Font font;
	private CellStyle header;
	private CellStyle styleOdd;
	private CellStyle styleOddCenter;
	private CellStyle styleOddTutar;
	private CellStyle styleOddNumber;
	private CellStyle styleEven;
	private CellStyle styleEvenCenter;
	private CellStyle styleEvenTutar;
	private CellStyle styleEvenNumber;
	...
	public VardiyaGun vardiyaBul(AylikPuantaj aylikPuantaj, VardiyaGun islemVardiyaGun) {
		...
		try {
			...
			if (vardiyaGun != null) {
				...
				fazlaMesailer = vardiyaGun.getFazlaMesailer();
			}
	...
	private void fazlaCalismaHazirla(List<Long> personelIdler) throws Exception {
		...
		if (!list.isEmpty()) {
			...
			for (VardiyaGun gun : list) {
				if (gun.getVardiyaSaat() == null || gun.getVardiyaSaat().getCalismaSuresi() > 0.0d)
			...
			if (!varMap.isEmpty()) {
				...
				boolean maxGeceCalismaSaatDurum = raporSecim.equals("maxGeceCalismaSaat");
				cal.setTime(basTarih);
				...
				while (bitTarih.getTime() >= cal.getTime().getTime()) {
					...
					for (AylikPuantaj aylikPuantaj : aylikPuantajList) {
						...
						} else if (maxGeceCalismaSaatDurum)
							aylikPuantaj.getVardiyalar().add(gun2);
	...
	public Double getCalismaSuresi(int hafta, AylikPuantaj aylikPuantaj) {
		...
		Double double1 = haftaCalismaMap.containsKey(key) ? (Double) haftaCalismaMap.get(key) : null;
		return double1;
	}
				...
				for (PersonelFazlaMesai personelFazlaMesai : list) {
					if (personelFazlaMesai.isOnaylandi()) {
						...
						if ((gun.getFazlaMesailer() != null) && (gun.getSonrakiVardiyaGun() != null)) {
							if ((gun.getVardiyaDateStr().equals("20230806")) || (gun.getVardiyaDateStr().equals("20230807")))
								logger.debug(gun.getId() + " " + gun.getVardiyaDateStr());
	...
	protected ByteArrayOutputStream fazlaMesaiExcelDevam(String gorevYeriAciklama, List<AylikPuantaj> list1) {
		...
		fontBold = ExcelUtil.createFont(wb, (short) 9, ExcelUtil.FONT_NAME, (short) 700);
		font = ExcelUtil.createFont(wb, (short) 8, ExcelUtil.FONT_NAME, (short) 400);

		header = ExcelUtil.getStyleHeader(wb);
		styleOdd = ExcelUtil.getStyleOdd(null, wb);
		styleOddCenter = ExcelUtil.getStyleOdd("alignCenter", wb);
		styleOddTutar = ExcelUtil.getStyleOdd("formatTutar", wb);
		styleOddNumber = ExcelUtil.getStyleOdd("formatNumber", wb);
		styleEven = ExcelUtil.getStyleEven(null, wb);
		styleEvenCenter = ExcelUtil.getStyleEven("alignCenter", wb);
		styleEvenTutar = ExcelUtil.getStyleEven("formatTutar", wb);
		styleEvenNumber = ExcelUtil.getStyleEven("formatNumber", wb);
	...
	private void sayfaAylikPuantajOlustur(List<AylikPuantaj> list, Workbook wb) {
		...
		List vardiyaGunPersonelList = new ArrayList();
		...
		boolean renk = true;
		CellStyle styleRedOddTutar = ExcelUtil.getStyleOdd("formatTutar", wb);
		CellStyle styleRedOddNumber = ExcelUtil.getStyleOdd("formatNumber", wb);
		CellStyle styleRedEvenTutar = ExcelUtil.getStyleEven("formatTutar", wb);
		CellStyle styleRedEvenNumber = ExcelUtil.getStyleEven("formatNumber", wb);
		...
		for (Iterator iter = list.iterator(); iter.hasNext();) {
			...
			CellStyle styleRedDouble = null;
			CellStyle styleRedNumber = null;
			if (renk) {
				styleGenel = styleOdd;
				...
				if (vardiyaHaftaList.isEmpty())
					for (VardiyaGun islemVardiyaGun : vardiyaGunList) {
						...
						if ((gun != null) && ((vardiya != null) || (maxToplamMesaiDurum))) {
							Double tutar = Double.valueOf(0.0D);
							Cell fmtCell = null;
							...
							if (tutar.doubleValue() > 0.0D) {
								if (PdksUtil.isDoubleValueNotLong(tutar))
									fmtCell = ExcelUtil.getCell(sheet, row, col++, !kirmizi ? styleDouble : styleRedDouble);
								else
									fmtCell = ExcelUtil.getCell(sheet, row, col++, !kirmizi ? styleNumber : styleRedNumber);
								fmtCell.setCellValue(tutar.doubleValue());
							} else {
								fmtCell = ExcelUtil.getCell(sheet, row, col++, styleCenter);
								fmtCell.setCellValue((!maxHaftaTatilCalismaGunDurum) || (vardiya == null) || (!vardiya.isHaftaTatil()) ? "" : vardiya.getVardiyaAciklama());
							}
							if (!maxToplamMesaiDurum) {
